...
QEMU_DEVICES+= -device virtio-blk-pci-non-transitional,drive=hdd,bus=rp2,id=blk1
QEMU_DEVICES+= -device virtio-blk-pci-non-transitional,drive=hdd2,bus=rp2,id=blk2
QEMU_DEVICES+= -device virtio-blk-pci-non-transitional,drive=hdd3,bus=rp2,id=blk3
QEMU_DEVICES+= -device qemu-xhci,bus=rp3,id=usbhost
QEMU_DEVICES+= -drive if=none,format=raw,file=$(QEMU_HARD_DRIVE_1),id=hdd
